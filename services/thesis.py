...
from schemas.thesis import AcademyYearResponse, BatchResponse, BatchSimpleResponse, SemesterResponse, ThesisCreate, ThesisResponse, ThesisUpdate

def create(db: Session, thesis: ThesisCreate, lecturer_id: uuid.UUID):
    creator = db.query(User).filter(User.id == lecturer_id,or_(User.user_type == 3, User.user_type == 1)  # Kiểm tra user_type là 3 hoặc 1
    ).first()
    if not creator:
